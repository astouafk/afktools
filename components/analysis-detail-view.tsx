...
export function AnalysisDetailView({ projectId, analysisId }: AnalysisDetailViewProps) {
  ...
  // ✅ CORRECTION - Ajouter project pour récupérer companyId
  const { data: project } = useProject(projectId)
  const { data: reports = [] } = useReports(projectId, project?.companyId || null)
  ...
  const fieldsWithContent = fields.map(field => ({
    ...
    isShort: (content[field.name] || '').length < 150
  }))
