...
import { getAnalysesByProject, getAnalysisById, updateAnalysis, createAnalysis, markAnalysisAsCompleted, deleteAnalysis } from '@/lib/services/analysis-service'
import { updateProjectProgress } from '@/lib/services/project-service'

// ✅ Passer companyId
export function useAnalyses(projectId: string | null, companyId: string | null) {
  return useQuery({
    queryKey: ['analyses', projectId, companyId],
    queryFn: () => getAnalysesByProject(projectId!, companyId!),
...
export function useCreateAnalysis() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['analyses', variables.projectId, variables.companyId] })
      queryClient.invalidateQueries({ queryKey: ['project', variables.projectId] })
    }
  })
}
